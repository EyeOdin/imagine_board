...
class ImagineBoard_Extension(Extension):
    ...
    def createActions(self, window):
        ...
        action_export_clipboard = window.createAction(EXTENSION_ID+"_export_from_clipboard", "Export from Clipboard", "tools/scripts/Imagine Board Menu")
        action_export_clipboard.triggered.connect(self.Export_Clipboard)

        action_export_canvas = window.createAction(EXTENSION_ID+"_export_from_canvas", "Export from Canvas", "tools/scripts/Imagine Board Menu")
        action_export_canvas.triggered.connect(self.Export_Canvas)
    ...
    def Export_Clipboard(self):
        self.SIGNAL_EXPORT_CLIPBOARD.emit(0)
    def Export_Canvas(self):
        self.SIGNAL_EXPORT_CANVAS.emit(0)
