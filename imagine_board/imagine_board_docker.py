...
imagine_board_version = "2022_05_14"
...
class ImagineBoard_Docker(DockWidget):
    ...
    def Variables(self):
        ...
        self.inside = False
        self.mode = 25
        self.foot = 5 + 15 + self.mode

        # Lists
        self.bool_recentdocuments = False
        self.bool_references = False
        self.list_recentdocuments = []
    ...
    def Connections(self):
        ...
        self.layout.mode.currentIndexChanged.connect(lambda: self.Menu_Mode(self.layout.mode.currentIndex(), 0))
        ...
        self.dialog.menu_recentdocuments.toggled.connect(self.List_RecentDocuments)
        self.dialog.menu_references.toggled.connect(self.List_References)
    ...
    def Modules(self):
        ...
        self.imagine_preview.Set_Label(self.layout.preview_view)
        ...
        self.imagine_preview.SIGNAL_PIN.connect(self.Reference_Add)
        ...
        self.imagine_preview.SIGNAL_CROP.connect(self.Preview_Crop)
        ...
        self.imagine_reference.SIGNAL_SAVE.connect(self.Reference_KRA_Save)
    ...
    def Settings(self):
        ...
        bool_recentdocuments = Krita.instance().readSetting("Imagine Board", "bool_recentdocuments", "")
        if bool_recentdocuments == "":
            Krita.instance().writeSetting("Imagine Board", "bool_recentdocuments", "False")
        else:
            self.dialog.menu_recentdocuments.setChecked( eval(bool_recentdocuments) )
        # Reference Images List
        bool_references = Krita.instance().readSetting("Imagine Board", "bool_references", "")
        if bool_references == "":
            Krita.instance().writeSetting("Imagine Board", "bool_references", "False")
        else:
            self.dialog.menu_references.setChecked( eval(bool_references) )
        # Directory File
        ...
        if ref_import == "":
            Krita.instance().writeSetting("Imagine Board", "ref_import", "False")
        else:
        ...
        #//
        #\\ References #####################################################
        references = Krita.instance().readSetting("Imagine Board", "list_references", "")
        if references == "":
            Krita.instance().writeSetting("Imagine Board", "list_references", "")
        else:
            self.Board_Loader(eval(references))
    ...
    def Menu_Mode(self, mode, foot):
        ...
        if mode == 0:
            ...
            if self.transparent == True:
                if (float == True and self.inside == False):
                    ...
                    self.setGeometry(size[0], size[1], size[2], size[3] - foot)
                else:
                    self.layout.progress_bar.setMaximumHeight(a)
                    self.layout.slider.setMaximumHeight(b)
                    self.layout.horizontal_buttons.setMaximumHeight(c)
                    self.setGeometry(size[0], size[1], size[2], size[3] + foot)
            else:
                self.layout.progress_bar.setMaximumHeight(a)
                self.layout.slider.setMaximumHeight(b)
            ...
            # QMovie
            self.Menu_AnimPanel(self.anim_qmovie)
        # Grid
        if mode == 1:
            # Module Containers
    ...
    # Window
    def Menu_Transparent(self):
        self.transparent = self.dialog.menu_transparent.isChecked()
        if self.isFloating() == True:
            self.Menu_Mode(self.menu_mode, 0)
        # Save
    ...
    def Index_Enable(self, bool):
        if self.slideshow_play == False:
            self.layout.slider.setEnabled(bool)
            self.layout.number.setEnabled(bool)
    ...
    def List_RecentDocuments(self):
        # Checks
        self.bool_recentdocuments = self.dialog.menu_recentdocuments.isChecked()
        self.dialog.menu_references.setChecked(False)
        # Watcher
        recentdocuments = krita.Krita.instance().recentDocuments()
        self.list_recentdocuments = []
        for i in range(0, len(recentdocuments)):
            if recentdocuments[i] != "":
                self.list_recentdocuments.append(recentdocuments[i])
                if self.bool_recentdocuments == True:
                    self.directory_watcher.addPath(recentdocuments[i])
                else:
                    self.directory_watcher.removePath(recentdocuments[i])
        # Finish
        self.Filter_Keywords(True)
        Krita.instance().writeSetting("Imagine Board", "bool_recentdocuments", str( self.bool_recentdocuments ))
        self.update()
    def List_References(self):
        # Exclude other Lists
        self.bool_references = self.dialog.menu_references.isChecked()
        self.dialog.menu_recentdocuments.setChecked(False)
        # Watcher
        for i in range(0, len(self.list_references)):
            if self.bool_references == True:
                self.directory_watcher.addPath(self.list_references[i]["path"])
            else:
                self.directory_watcher.removePath(self.list_references[i]["path"])
        # Finish
        self.Filter_Keywords(True)
        Krita.instance().writeSetting("Imagine Board", "bool_references", str( self.bool_references ))
        self.update()
        ...
        if self.widget_display == True:
            ...
            if self.directory_path != "":
                if self.bool_recentdocuments == True:
                    # Recent Documents
                    files = []
                    for i in range(0, len(self.list_recentdocuments)):
                        files.append(self.list_recentdocuments[i])
                    files.reverse()
                    count = len(files)
                elif self.bool_references == True:
                    # Recent Documents
                    files = []
                    for i in range(0, len(self.list_references)):
                        files.append(self.list_references[i]["path"])
                    files.reverse()
                    count = len(files)
                else:
                    # Directory
                ...
                for k in range(0, len(keywords)):
                    for i in range(0, count):
                        if (self.bool_recentdocuments == True or self.bool_references == True):
    ...
    def Display_Reference(self):
        if self.menu_mode == 2:
            self.Board_Loader(self.list_references)
    ...
    def Preview_Crop(self, SIGNAL_CROP):
        self.crop = SIGNAL_CROP
    ...
    def Grid_Favorite(self, SIGNAL_PIN):
        ...
        if (self.images_found == True and num < self.preview_max):
            ...
            if pin not in self.list_references:
    ...
    def Reference_Add(self, path):
        ...
        area = width * height
        try:ratio = width / height
        except:ratio = 0
        if self.clip_state == True:
            crop = [self.crop[0], self.crop[1], self.crop[2], self.crop[3]]
        else:
            crop = [0,0,1,1]
        text = ""
        ...
        self.list_references.append(pin)
        self.imagine_reference.Pin_Add(pin)

    def Reference_Load(self, SIGNAL_LOAD):
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            try:
                list_references = eval(Krita.instance().activeDocument().annotation("imagineboard_annotations"))
                self.Board_Loader(list_references)
            except:
                pass
    def Board_Loader(self, lista):
        ...
        for i in range(0, len(lista)):
            ...
            try:
                # Dictionary Saved
                pin["path"] = lista[i]["path"]
                try:   pin["px"] = lista[i]["px"]
                except:pin["px"] = 10
                try:   pin["py"] = lista[i]["py"]
                except:pin["py"] = 10
                try:   pin["dw"] = lista[i]["dw"]
                except:pin["dw"] = QPixmap(pin["path"]).width()
                try:   pin["dh"] = lista[i]["dh"]
                except:pin["dh"] = QPixmap(pin["path"]).height()
            except TypeError:
                # List Saved (old method)
                pin["path"] = lista[i][0]
                try:   pin["px"] = lista[i][1]
                except:pin["px"] = 10
                try:   pin["py"] = lista[i][2]
                except:pin["py"] = 10
                try:   pin["dw"] = lista[i][3]
                except:pin["dw"] = QPixmap(pin["path"]).width()
                try:   pin["dh"] = lista[i][4]
                except:pin["dh"] = QPixmap(pin["path"]).height()
            if pin["path"] != "":
                # Independant Information
                try:   pin["ang"] = lista[i]["ang"]
                except:pin["ang"] = self.Math_2D_Points_Lines_Angle(pin["dw"],0, 0,0, pin["dw"], pin["dh"])
                try:   pin["per"] = lista[i]["per"]
                except:pin["per"] = (2 * pin["dw"]) + (2 * pin["dh"])
                try:   pin["are"] = lista[i]["are"]
                except:pin["are"] = pin["dw"] * pin["dh"]
                try:   pin["rat"] = lista[i]["rat"]
                except:pin["rat"] = pin["dw"] / pin["dh"]
                try:   pin["cro"] = lista[i]["cro"]
                except:pin["cro"] = [0,0,1,1]
                try:   pin["tex"] = lista[i]["tex"]
                except:pin["tex"] = ""
                try:   pin["egs"] = lista[i]["egs"]
                except:pin["egs"] = False
                try:   pin["efx"] = lista[i]["efx"]
                except:pin["efx"] = False
                try:   pin["efy"] = lista[i]["efy"]
                except:pin["efy"] = False
                pin["qpixmap"] = None
                self.list_references.append(pin)
        if len(self.list_references) > 0:
            self.imagine_reference.Board_Load(self.list_references)

    def Reference_KRA_Save(self, SIGNAL_SAVE):
        # Signal
        self.kra_bind = SIGNAL_SAVE[0]
        self.list_references = SIGNAL_SAVE[1]
        data = str(self.list_references)
        # Kritarc
        Krita.instance().writeSetting("Imagine Board", "list_references", data )
        # Save Annotations
    ...
    def Function_NewPath(self, SIGNAL_NEWPATH):
        ...
        # Change References
        for i in range(0, len(self.list_references)):
            if self.list_references[i]["path"] == path_i:
                self.list_references[i]["path"] = path_new
    ...
    def enterEvent(self, event):
        ...
        if self.isFloating() == True:
            self.Menu_Mode(self.layout.mode.currentIndex(), self.foot)
        self.update()
...
class Thread_Function(QThread):
    ...
    def String_ReName(self, path, key, number, iter, mode):
        if len(key) == 0:
            ...
            # Generate Names
            if (mode == "order" or mode == "missing" or mode == "age"):
