...
class ImagineBoard_Docker(DockWidget):
    ...
    def User_Interface(self):
        ...
        self.image_path = os.path.join(self.directory_plugin, "IMAGE\\")
    ...
    def Modules(self):
        ...
        self.imagine_preview.SIGNAL_COLOR.connect(self.Color_Picker)
        ...
        self.imagine_grid.SIGNAL_COLOR.connect(self.Color_Picker)
        ...
        self.imagine_reference.SIGNAL_NEUTRAL.connect(self.Click_Neutral)
        # Reference
        self.imagine_reference.SIGNAL_COLOR.connect(self.Color_Picker)
        ...
        #region Database
        if development == True:
            self.imagine_database = ImagineBoard_Database(self.layout.imagine_database)
            self.imagine_database.Database_Run(self.directory_plugin)

            pin = {
                '_path': 'C:\\Users\\EyeOd\\AppData\\Roaming\\krita\\pykrita\\imagine_board\\TEST FILES\\GREEN\\pexels-ali-kazal-10885300.jpg',
                '_index': 0,
                '_active': False,
                '_select': False,
                '_pack': False,
                '_text': None,
                '_color': '#595959',
                '_pis': 13.231349999999997,
                '_pir': 10.0,
                '_ox': 101.85623,
                '_oy': 92.36524,
                '_rx': 0,
                '_ry': 0,
                '_rn': 56.309932474020215,
                '_ro': 0.0,
                '_ss': 79.5105181144,
                '_sr': 1.0,
                '_dx': 57.75173,
            ...
            self.imagine_database.Database_Pin_New(pin)
        ...
        if thread_function == True:
            ...
            self.thread_function = Thread_Function(self.dialog.menu_function_text)
    ...
    def Mode_Index(self, index):
        ...
        if index == 0:
            ...
            self.layout.folder.setMaximumWidth(width)
            self.layout.slideshow.setMaximumWidth(width)
            # Enable
        ...
        if index == 1:
            ...
            self.layout.folder.setMaximumWidth(width)
            self.layout.thread.setMaximumWidth(width)
            # Enable
            self.Index_Enable(True)
        # Reference
        if index == 2:
            ...
            self.layout.undo.setMaximumWidth(width)
            self.layout.redo.setMaximumWidth(width)
            # Enable
        ...
        if development == True:
            if index == 3:
                ...
                self.layout.mode.setIcon( self.qicon_reference )
                # Module
                self.layout.imagine_database.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
                self.layout.imagine_database.setMaximumHeight(qt_max)
                self.layout.undo.setMaximumWidth(width)
                self.layout.redo.setMaximumWidth(width)
                # Enable
                self.Index_Enable(False)
    ...
    def Display_Shrink(self):
        ...
        self.layout.imagine_database.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Fixed)
        # Height
    ...
    def Menu_Mode_Press(self, event):
        ...
        if development == True:cmenu_database = cmenu.addAction("Database")
        # Icons
        cmenu_preview.setIcon( self.qicon_preview )
        cmenu_grid.setIcon( self.qicon_grid )
        cmenu_reference.setIcon( self.qicon_reference )
        if development == True:cmenu_database.setIcon( self.qicon_reference )
    ...
    def Update_Size(self):
        ...
        if check == True:
            ...
            if development == True:self.imagine_database.Set_Size_Corner(self.layout.imagine_database.width(), self.layout.imagine_database.height())
            ...
            if development == True:self.imagine_database.Set_Size_Middle(self.layout.imagine_database.width(), self.layout.imagine_database.height())
        ...
        try:
            ...
            for i in range(0, len(dockers)):
                if dockers[i].objectName() == self.MODULE_pigmento_pyid:
                    ...
                    self.imagine_grid.Set_Pigmento(True)
                    self.imagine_reference.Set_Pigmento(True)
                    break
            else:
    ...
    # Color
    def Color_Picker(self, SIGNAL_COLOR):
        # QImage RGB values
        ...
        if self.MODULE_pigmento_bool == True:
            ...
            if self.mode_index == 1:
                self.imagine_grid.Set_Color_Display(color["d_rgb_1"], color["d_rgb_2"], color["d_rgb_3"])
            if self.mode_index == 2:
                self.imagine_reference.Set_Color_Display(color["d_rgb_1"], color["d_rgb_2"], color["d_rgb_3"])
        else:
        ...
        if self.mode_index == 1:
            self.imagine_grid.Set_Color_Display(kac_1, kac_2, kac_3)
        if self.mode_index == 2:
            self.imagine_reference.Set_Color_Display(kac_1, kac_2, kac_3)
    ...
    def Information_Read(self):
        ...
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            ...
            try:
                ...
                # Calculations
                path = ad.fileName()
                file_name = str( os.path.basename( ad.fileName() ) )
                ...
                if zipfile.is_zipfile(path):
                    ...
                    for i in range(len(splits)):
                        ...
                        if index.startswith(header):
                            ...
                            break
                    numbers = numbers[1:-1]
                    self.krita_version = "Krita version " + str(numbers)
            except:
                pass
        else:
            self.Money_Rate(0)
        ...
        self.dialog.info_description.setText(self.krita_version) # Abstract is Description inside Krita
        # Time
    ...
    def Information_Save(self):
        ...
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            ...
            new_description = self.krita_version
    ...
    def ThreadFunction_Run(self, SIGNAL_PATH):
        ...
        if destination == "":
            ...
            if exists == False:
                destination = ""
                QMessageBox.information(QWidget(), i18n("Warnning"), i18n("Invalid destination path.\nDefault path is display directory."))
        ...
        if (len(path_list) > 0 and destination != ""):
            ...
            # Thread
            self.thread_function.Variables_Run(operation, string, number, path_list, destination, self.image_path, file_normal)
    ...
    def ThreadFunction_String(self, SIGNAL_STRING):
        self.dialog.menu_function_text.setText(SIGNAL_STRING)
    ...
    def Window_Closed(self):
        if development == True:self.imagine_database.Database_Disconnect()
...
if thread_function == True:tf_class = QThread
else:tf_class = QWidget
class Thread_Function(tf_class):
    ...
    def Variables_Run(self, operation, string, number, path_list, destination, image_path, file_normal):
        # Reset
        self.Variables_Reset()
        # Variables
    ...
    def run(self):
        ...
        # Clean
        self.Clean_Web(self.image_path, self.path_list)
    ...
    def Clean_Web(self, image_path, path_list):
        for i in range(len(path_list)):
            try:
                path_i = os.path.normpath( path_list[i] )
                check = path_i.startswith( image_path )
                if check == True:
...
- Create color palette from image (resize image to go faster)
