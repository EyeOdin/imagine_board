...
class ImagineBoard_Docker(DockWidget):
    ...
    def Progress_Max(self, max):
        self.layout.progress_bar.setMaximum(max)
        self.layout.progress_bar.update()
            ...
            if self.images_found == True:
                ...
                if QPixmap(path).isNull() == False:
                    if extension in file_anim:
                        self.imagine_preview.Set_Anim_Preview(path, self.anim_playpause)
                    else:
                        self.imagine_preview.Set_QPixmap_Preview(path)
                elif extension in file_compress:
                    self.Namelist_Start(path)
            else:
                self.imagine_preview.Set_Default()
    ...
    def Thumbnail_Start(self, SIGNAL_LOAD):
        ...
        self.layout.index_number.setMaximum(16777215)
...
class Thread_NameList(QThread):
    ...
    def run(self):
        ...
        try:
            if zipfile.is_zipfile(self.path):
                ...
                for i in range(0, length):
                    self.SIGNAL_PROGRESS_VALUE.emit(i+1)
                    try:
                        archive_open = archive.open(name_list[i])
                        archive_read = archive_open.read()
                        qimage = QImage()
        ...
        self.SIGNAL_NAMELIST.emit([self.path, image_name])
