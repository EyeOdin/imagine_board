...
class ImagineBoard_Docker(DockWidget):
    ...
    def Settings(self):
        try:
            ...
            self.dialog.menu_slideshow_time.setValue(         float(Krita.instance().readSetting("Imagine Board", "slideshow_time", "")) )
    ...
    def Menu_SlideShow_Time(self, time):
        factor = 1000
        self.slideshow_time = time * factor # 1 x 1000 = 1second
        Krita.instance().writeSetting("Imagine Board", "slideshow_time", str( self.slideshow_time / factor ))

        minutes = math.trunc(time / 60)
        self.dialog.menu_slideshow_time.setPrefix( str(minutes) + "m, " )
    ...
    def Thumbnail_Start(self, SIGNAL_LOAD):
        ...
        # Dynamic Range
        left = self.grid_page * self.grid_horz
        right = (self.grid_page + self.grid_vert) * self.grid_horz
    ...
    def Hover_DragDrop(self, SIGNAL_DRAG):
        index = self.grid_to_prev(self.grid_page, self.grid_horz, SIGNAL_DRAG[0], SIGNAL_DRAG[1])
    ...
    def Grid_Name(self, SIGNAL_NAME):
        num = self.grid_to_prev(self.grid_page, self.grid_horz, SIGNAL_NAME[0], SIGNAL_NAME[1])
        if num < self.preview_max:
