...
class ImagineBoard_Docker(DockWidget):
    ...
    def Settings(self):
        ...
        # Autosave KRA
        annotation_kra = Krita.instance().readSetting("Imagine Board", "annotation_kra", "")
        if annotation_kra == "":
            Krita.instance().writeSetting("Imagine Board", "annotation_kra", "False")
        else:
            self.dialog.annotation_kra_save.setChecked( eval(annotation_kra) )
        # Autosave File
        annotation_file = Krita.instance().readSetting("Imagine Board", "annotation_file", "")
        if annotation_file == "":
            Krita.instance().writeSetting("Imagine Board", "annotation_file", "False")
        else:
            self.dialog.annotation_file_save.setChecked( eval(annotation_file) )
    ...
    def Menu_Index(self, index):
        ...
        # update cycle
        if self.mode_index != index: # After a search with null results or reference board change, this ensure other modes update
    ...
    def Watcher_Add(self):
        ...
        for r in range(0, len(self.list_pin_ref)):
            ref_r = self.list_pin_ref[r]["path"]
            text_r = self.list_pin_ref[r]["text"]
            if (ref_r != None and text_r == None):
        ...
        if self.list_active == "Recent Documents":
            for d in range(0, len(self.list_recentdocument)):
                ref_d = self.list_recentdocument[d]
    ...
    def enterEvent(self, event):
        ...
        self.inside = True
        self.Transparent_Shift()
        # Preview and Grid (Helps with the Compressed load error)
        if self.widget_startup == False:
            self.widget_startup = True
            self.Folder_Load(self.directory_path)
