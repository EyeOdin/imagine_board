...
imagine_board_version = "2022_04_22"
...
class ImagineBoard_Docker(DockWidget):
    ...
    def Connections(self):
        ...
        # Dialog Information
        self.dialog.info_list.clicked.connect(self.Info_Document)
        # Dialog Connections
    ...
    def Settings(self):
        try:
            ...
            # Dialog Display
            self.dialog.menu_recent.setChecked( eval(Krita.instance().readSetting("Imagine Board", "recent_documents", "")) )
            ...
            self.dialog.menu_cache_clean.setValue(  int(Krita.instance().readSetting("Imagine Board", "cache_clean", "")) )
            self.dialog.menu_cache_load.setValue(   int(Krita.instance().readSetting("Imagine Board", "cache_load", "")) )
            self.dialog.menu_cache_thread.setValue( int(Krita.instance().readSetting("Imagine Board", "cache_thread", "")) )
            self.dialog.menu_thumbnails.setValue(   int(Krita.instance().readSetting("Imagine Board", "thumbnail_limit", "")) )
            ...
            # References
            self.references = eval(Krita.instance().readSetting("Imagine Board", "references", ""))
    ...
    def Menu_Mode(self, mode):
        ...
        self.Display_Shrink()
        self.AnimPanel_Shrink()
        self.imagine_reference.Set_Select(False)
        # Preview
        if mode == 0:
    ...
    def Reference_Add(self, path):
        ...
        pin = [os.path.normpath(path), location_x, location_y, width, height, angle]
    ...
    def Info_Document(self):
        ...
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            ...
            # Check if list size remains valid
            if len(root) == 20:
                self.dialog.info_list.setStyleSheet("#info_list{background-color: rgba(50, 0, 0, 50);}")
            else:
                self.dialog.info_list.setStyleSheet("#info_list{}")

            # Date Edits
            creator_fl_name = str(creator_first_name)+" "+str(creator_last_name)
            ...
            date_delta = str(yearHH)+"y "+str(monthHH).zfill(2)+"m "+str(dayHH).zfill(2)+"d"
            ...
            if editing_time == "Editing Time" or editing_time is None:
                ...
                secondNN = minuteRR
                active_time = str(yearNN)+"y "+str(monthNN).zfill(2)+"m "+str(dayNN).zfill(2)+"d : "+str(hourNN).zfill(2)+"h "+str(minuteNN).zfill(2)+"m "+str(secondNN).zfill(2)+"s"

        # Deplay XML items on the tab
        self.dialog.info_list.clear()
        self.dialog.info_list.addItem("Title           : " + str(title))
        self.dialog.info_list.addItem("Description     : " + str(description))
        self.dialog.info_list.addItem("Subject         : " + str(subject))
        self.dialog.info_list.addItem("Abstract        : " + str(abstract))
        self.dialog.info_list.addItem("Keyword         : " + str(keyword))
        self.dialog.info_list.addItem("Initial Creator : " + str(initial_creator))
        self.dialog.info_list.addItem("Edit Cycles     : " + str(editing_cycles))
        self.dialog.info_list.addItem("Edit Time       : " + str(editing_time))
        self.dialog.info_list.addItem("Date            : " + str(date))
        self.dialog.info_list.addItem("Creation Date   : " + str(creation_date))
        self.dialog.info_list.addItem("Language        : " + str(language))
        self.dialog.info_list.addItem("License         : " + str(license))
        self.dialog.info_list.addItem("")
        self.dialog.info_list.addItem("Nick Name       : " + str(nick_name))
        self.dialog.info_list.addItem("Creator Name    : " + str(creator_fl_name))
        self.dialog.info_list.addItem("Initial         : " + str(initial))
        self.dialog.info_list.addItem("Author Title    : " + str(author_title))
        self.dialog.info_list.addItem("Position        : " + str(position))
        self.dialog.info_list.addItem("Company         : " + str(company))
        self.dialog.info_list.addItem("Contact Type    : " + str(contact_type))
        self.dialog.info_list.addItem("")
        self.dialog.info_list.addItem("Creation Date   : " + str(cd3))
        self.dialog.info_list.addItem("Creation Delta  : " + str(date_delta))
        self.dialog.info_list.addItem("Edit Cycles     : " + str(editing_time))
        self.dialog.info_list.addItem("Edit Time       : " + str(active_time))
