...
class ImagineBoard_Docker(DockWidget):
    ...
    def Modules(self):
        ...
        self.imagine_grid.SIGNAL_PIN.connect(self.Reference_Add)
    ...
    def Click_Neutral(self, SIGNAL_NEUTRAL):
        if SIGNAL_NEUTRAL != "":
            # try:
            #     name = self.found_path[self.preview_index]
            # except:
            #     name = ""
            # self.layout.search.setPlaceholderText(os.path.basename(name))
        ...
        if self.mode_index == 0:
            ...
            if self.images_found == True:
                ...
                if QPixmap(image_path).isNull() == False:
                    extension = os.path.splitext(image_path)[1][1:] # .ext
                    if extension in file_anim:
                        self.imagine_preview.Set_Sequence_Preview(image_path, self.anim_playpause)
                    else:
                        self.imagine_preview.Set_QPixmap_Preview(image_path)
                else:
                    self.imagine_preview.Set_Default_Preview(image_path)
            else:
    ...
    def Preview_Color(self, SIGNAL_COLOR):
        ...
        # self.layout.search.setPlaceholderText( str(int(kac_1*255))+" "+str(int(kac_2*255))+" "+str(int(kac_3*255)) )
        # Pigmento Apply
