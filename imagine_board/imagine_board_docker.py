...
class ImagineBoard_Docker(DockWidget):
    ...
    def Modules(self):
        ...
        self.imagine_reference.SIGNAL_REFERENCE.connect(self.Reference_Insert)
        ...
        #endregion
        #region ColorPicker
        self.colorpicker_widget = QWidget(None)
        self.colorpicker_widget.setGeometry(0, 0, 150, 150)
        self.colorpicker_widget.show()
        self.colorpicker_widget.setWindowFlags( Qt.WindowStaysOnTopHint | Qt.WindowTransparentForInput | Qt.FramelessWindowHint | Qt.NoDropShadowWindowHint )
        self.colorpicker_widget.setAttribute(Qt.WA_TranslucentBackground, True)

        self.colorpicker_display = ColorPicker_Display(self.colorpicker_widget)
    ...
    def Click_Press(self, SIGNAL_PRESS):
        # Parsing
        press = SIGNAL_PRESS["press"]
        picker = SIGNAL_PRESS["picker"]
        px = SIGNAL_PRESS["px"]
        py = SIGNAL_PRESS["py"]
        size = SIGNAL_PRESS["size"]

        # Function
        self.Function_Operation()

        # Color Picker
        if (press == True and picker == True):
            self.colorpicker_widget.setParent(None)
            self.colorpicker_widget.setGeometry(px, py, size, size)
        else:
            self.colorpicker_widget.setParent(self)
            self.colorpicker_widget.setGeometry(0, 0, 0, 0)
        self.colorpicker_widget.show()
    def Text_Display(self, SIGNAL_TEXT):
        self.layout.search.setPlaceholderText(SIGNAL_TEXT)

    # Color
    def Color_Picker(self, SIGNAL_COLOR):
        # QImage RGB values
        press = SIGNAL_COLOR["press"]
        ...
        # Display Color
        self.colorpicker_display.Set_Color( kac_1, kac_2, kac_3 )
        if press == False:
            self.colorpicker_display.Set_Previous( kac_1, kac_2, kac_3 )
