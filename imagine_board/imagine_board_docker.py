...
imagine_board_version = "2022_08_16"
...
class ImagineBoard_Docker(DockWidget):
    ...
    def Variables(self):
        ...
        # Export
        self.watermark_bool = False
        self.watermark_path = ""
        self.watermark_qpixmap = QPixmap()
    ...
    def Modules(self):
        ...
        self.imagine_preview.SIGNAL_PIN_PATH.connect(self.Reference_Add)
        ...
        self.imagine_preview.SIGNAL_ANIM_ENABLE.connect(self.Menu_AnimEnable)
        #//
        #\\ Grid
        ...
        self.imagine_grid.SIGNAL_PIN_PATH.connect(self.Reference_Add)
        ...
        self.imagine_reference.SIGNAL_SAVE.connect(self.Reference_Save)
    ...
    def Style(self):
        ...
        self.layout.number.setToolTip("Image Index")
    ...
    def Settings(self):
        ...
        if mode_index == "":
            ...
            self.Menu_Index(int(mode_index))
        # Directory Path
    ...
    def Transparent_Shift(self):
        ...
        self.layout.slider.setMaximumHeight(sl)
    ...
    def Menu_AnimEnable(self, boolean):
        self.layout.anim_playpause.setEnabled(boolean)
        if boolean == True:
            self.layout.anim_playpause.setIcon(Krita.instance().icon('animation_pause'))
        if boolean == False:
            self.layout.anim_playpause.setIcon(Krita.instance().icon('animation_stop'))
    # Window
    ...
    def Compressed_Thumbnail(self, path):
    ...
    def Folder_Open(self):
        ...
        if (directory_path != "" and directory_path != "." and self.directory_path != directory_path):
            self.Folder_Changer(directory_path)
    def Folder_Changer(self, directory_path):
        self.directory_path = directory_path
        ...
        if self.list_active == "Recent Documents":
            ...
            for i in range(0, len(recentdocuments)):
                if recentdocuments[i] != "":
                    ...
                    self.Watcher_Update()
        if self.list_active == "References":
        ...
        if self.widget_display == True:
            ...
            if self.directory_path != "":
                if self.list_active == "Recent Documents":
                    ...
                    files.reverse()
                    count = len(files)
                elif self.list_active == "References":
                    files = []
                    for i in range(0, len(self.list_references)):
                ...
                for k in range(0, len(keywords)):
                    for i in range(0, count):
                        if (self.list_active == "Recent Documents" or self.list_active == "References"):
    ...
    def Display_Update(self):
        ...
        # Update
        if self.images_found == True:
            name = os.path.basename(self.found_path[self.preview_index])
    ...
    def Display_Preview(self):
        if self.mode_index == 0:
            ...
            if self.images_found == True:
                path = self.found_path[self.preview_index]
                extension = self.Path_Extension(path)
                if QPixmap(path).isNull() == False:
                    if extension in file_anim:
                        self.imagine_preview.Set_Sequence_Preview(path, self.anim_playpause)
                    else:
                        self.imagine_preview.Set_QPixmap_Preview(path)
                elif extension in file_compress:
                    self.imagine_preview.Set_ZIP_Preview(path, file_normal)
            else:
                self.imagine_preview.Set_Default_Preview("")
                    ...
                    for h in range(0, self.grid_horz):
                        ...
                        try:
                            ...
                            if extension in file_compress:
                                thumbnail = self.Compressed_Thumbnail(path)
    ...
    def Thumbnail_Start(self, SIGNAL_LOAD):
        ...
        self.layout.search.setPlaceholderText("Loading")
    ...
    def Preview_Color(self, SIGNAL_COLOR):
        ...
        if self.MODULE_pigmento_bool == True:
            ...
            self.imagine_preview.Set_Color_Display(color["disp_1"], color["disp_2"], color["disp_3"])
        else:
    ...
    def Board_Loader(self, lista):
        ...
        for i in range(0, len(lista)):
            ...
            if pin["path"] != "":
                ...
                if (width != 0 and height != 0):
                    # Move
                    try:     pin["ox"] = lista[i]["ox"]
                    except:  pin["ox"] = ox
                    try:     pin["oy"] = lista[i]["oy"]
                    except:  pin["oy"] = oy
                    try:     pin["rn"] = lista[i]["rn"]
                    except:  pin["rn"] = Trig_2D_Points_Lines_Angle(width,0, 0,0, width, height)
                    try:     pin["ro"] = lista[i]["ro"]
                    except:  pin["ro"] = 0
                    # Rotation
                    try:     pin["rx"] = lista[i]["rx"]
                    except:  pin["rx"] = 0
                    try:     pin["ry"] = lista[i]["ry"]
                    except:  pin["ry"] = 0
                    # Scale
                    try:     pin["ss"] = lista[i]["ss"]
                    except:  pin["ss"] = Trig_2D_Points_Distance(0,0, width,height) * 0.5
                    try:     pin["sr"] = lista[i]["sr"]
                    except:  pin["sr"] = 1
                    # Draw
                    try:     pin["dx"] = lista[i]["dx"]
                    except:  pin["dx"] = ox - (width * 0.5)
                    try:     pin["dy"] = lista[i]["dy"]
                    except:  pin["dy"] = oy - (height * 0.5)
                    try:     pin["dw"] = lista[i]["dw"]
                    except:  pin["dw"] = width
                    try:     pin["dh"] = lista[i]["dh"]
                    except:  pin["dh"] = height
                    try:     pin["dxw"] = lista[i]["dxw"]
                    except:  pin["dxw"] = ox + (width * 0.5)
                    try:     pin["dyh"] = lista[i]["dyh"]
                    except:  pin["dyh"] = oy + (height * 0.5)
                    try:     pin["cl"] = lista[i]["cl"]
                    except:  pin["cl"] = 0
                    try:     pin["cr"] = lista[i]["cr"]
                    except:  pin["cr"] = 1
                    try:     pin["ct"] = lista[i]["ct"]
                    except:  pin["ct"] = 0
                    try:     pin["cb"] = lista[i]["cb"]
                    except:  pin["cb"] = 1
                    # Bounding Box
                    try:     pin["bl"] = lista[i]["bl"]
                    except:  pin["bl"] = ox - (width * 0.5)
                    try:     pin["br"] = lista[i]["br"]
                    except:  pin["br"] = oy - (height * 0.5)
                    try:     pin["bt"] = lista[i]["bt"]
                    except:  pin["bt"] = ox + (width * 0.5)
                    try:     pin["bb"] = lista[i]["bb"]
                    except:  pin["bb"] = oy + (height * 0.5)
                    # Sorting
                    try:     pin["active"] = lista[i]["active"]
                    except:  pin["active"] = False
                    try:     pin["select"] = lista[i]["select"]
                    except:  pin["select"] = False
                    try:     pin["perimeter"] = lista[i]["perimeter"]
                    except:  pin["perimeter"] = (2*width) + (2*height)
                    try:     pin["area"] = lista[i]["area"]
                    except:  pin["area"] = width * height
                    try:     pin["ratio"] = lista[i]["ratio"]
                    except:  pin["ratio"] = width / height
                    # Text
                    try:     pin["text"] = lista[i]["text"]
                    except:  pin["text"] = ""
                    # Edits
    ...
    def Reference_Save(self, SIGNAL_SAVE):
        # Signal
    ...
    def Watcher_Add(self):
        ...
        # Add References
        for r in range(0, len(self.list_references)):
            ref_r = self.list_references[r]["path"]
            dir_r = os.path.dirname(ref_r)
            if (dir_r != dir_p and ref_r not in path_list):
                path_list.append(ref_r)
        # Add Recent Documents
        if self.list_active == "Recent Documents":
            for d in range(0, len(self.list_recentdocuments)):
                ref_d = self.list_recentdocuments[d]
                dir_d = os.path.dirname(ref_d)
                if (dir_d != dir_p and ref_d not in path_list):
                    path_list.append(ref_d)
        # Watch these Files
        if len(path_list) > 0:
        ...
        for r in range(0, len(self.list_references)):
            ref_r = self.list_references[r]["path"]
            item = QListWidgetItem(ref_r)
            if ref_r in path_list:
        ...
        if self.list_active == "Recent Documents":
            for d in range(0, len(self.list_recentdocuments)):
                ref_d = self.list_recentdocuments[d]
                item = QListWidgetItem(ref_d)
                if ref_d in path_list:
                    qicon = Krita.instance().icon('document-new')
                else:
                    qicon = Krita.instance().icon('paintbrush')
                item.setIcon( qicon )
            ...
            info_string = """<?xml version="1.0" encoding="UTF-8"?>
                ...
                <document-info xmlns="http://www.calligra.org/DTD/document-info">
                 <about>
                  ...
                  <language>""" + self.info["language"] + """</language>
