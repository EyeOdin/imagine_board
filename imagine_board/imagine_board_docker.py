...
class ImagineBoard_Docker(DockWidget):
    ...
    def Read_Layout(self):
        ...
        if references == "":
            ...
            # Exclude non valid Paths
            self.list_pin_ref = []
            for i in range(0, len(list_pin_ref)):
                path = list_pin_ref[i]["path"]
                exists = os.path.exists(path)
                if exists == True:
                    self.list_pin_ref.append(list_pin_ref[i])
            # Save
