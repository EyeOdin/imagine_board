...
class ImagineBoard_Docker(DockWidget):
    ...
    def Menu_Mode_Wheel(self, event):
        ...
        if event.modifiers() == QtCore.Qt.NoModifier:
            ...
            if (value == -1 or value == 1):
                self.Menu_Index( Limit_Range(self.mode_index + value, 0, 2) )
    def Menu_Folder(self, event):
        # Variables
        directory = self.directory_path
        len_current = 8 * len( directory )
        ...
        # Actions
        parent = "... " + str( os.path.basename( os.path.dirname( directory ) ) )
        cmenu.addSection( directory )
        cmenu_parent = cmenu.addAction( parent )
        cmenu.addSection( " " )
        actions = {}
        for i in range(0, len(folders)):
            string = "\\ " + str( os.path.basename( folders[i] ) )
    ...
    def Reference_Insert(self, SIGNAL_REF):
        ...
        if (text == None or text == ""):
            ...
            width = int( text_rect.width() )
            height = int( text_rect.height() )
