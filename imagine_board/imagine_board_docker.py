...
class ImagineBoard_Docker(DockWidget):
    ...
    def Settings(self):
        ...
        if transparent == "":
            Krita.instance().writeSetting("Imagine Board", "transparent", "")
        else:
            self.dialog.menu_transparent.setChecked( bool(transparent) )
        # Recent Documents List
        bool_recentdocuments = Krita.instance().readSetting("Imagine Board", "bool_recentdocuments", "")
        if bool_recentdocuments == "":
            Krita.instance().writeSetting("Imagine Board", "bool_recentdocuments", "")
        else:
            self.dialog.menu_recentdocuments.setChecked( bool(bool_recentdocuments) )
        # Reference Images List
        bool_references = Krita.instance().readSetting("Imagine Board", "bool_references", "")
        if bool_references == "":
            Krita.instance().writeSetting("Imagine Board", "bool_references", "")
        else:
            self.dialog.menu_references.setChecked( bool(bool_references) )
        # Directory File
        ...
        if references == "":
            ...
            self.list_references = eval(references)
    ...
    def Menu_Function_Operation(self):
        ...
        if operation == "rename_missing":
            text = "RENAME MISSING (s, n)"
    ...
    def List_References(self):
        ...
        for i in range(0, len(self.list_references)):
            if self.bool_references == True:
                self.directory_watcher.addPath(self.list_references[i][0])
            else:
                self.directory_watcher.removePath(self.list_references[i][0])
        # Finish
        self.Filter_Keywords(True)
        ...
        if self.widget_display == True:
            ...
            if self.directory_path != "":
                if self.bool_recentdocuments == True:
                    ...
                    files = []
                    for i in range(0, len(self.list_references)):
                        files.append(self.list_references[i][0])
                    files.reverse()
                    count = len(files)
                else:
                    # Directory
    ...
    def Reference_Add(self, path):
        ...
        if self.clip_state == True:
            ...
            crop = [0,0,1,1]
        pin = [os.path.normpath(path), location_x, location_y, width, height, angle, perimeter, area, ratio, crop]
        # pin = {
    ...
    def Reference_Load(self, SIGNAL_LOAD):
        if ((self.canvas() is not None) and (self.canvas().view() is not None)):
            try:
                self.list_references = eval(Krita.instance().activeDocument().annotation("imagineboard_annotations"))
                self.imagine_reference.Board_Load(self.list_references)
            except:
                pass
    def Reference_Update(self, SIGNAL_UPDATE):
        ...
        self.list_references = SIGNAL_UPDATE[1]
    ...
    def Function_NewPath(self, SIGNAL_NEWPATH):
        ...
        # Change References
        for i in range(0, len(self.list_references)):
            if self.list_references[i][0] == path_i:
                self.list_references[i][0] = path_new
...
class Thread_Function(QThread):
    ...
    def run(self):
        ...
        if self.operation != ">>":
            ...
            rename = ["key_add", "key_replace", "key_remove", "key_clean", "rename_order", "rename_missing", "rename_age", "rename_random"]
    ...
    def File_Cycle(self, total, path, key, number):
        ...
        if self.operation == "rename_age":
            ...
            age_2 = []
            for i in range(0, total):
                age_2.append(age_1[i])
            age_2.sort()
            # QtCore.qDebug(str(age_1))
            # QtCore.qDebug(str(age_2))
        ...
        for i in range(0, total):
            ...
            # if self.operation == "rename_missing": # Rename
            #     # path_new = self.String_ReName(path_i, key, number, i, "missing")
            #     path_new = "C:\\Users\\EyeOd\\Desktop\\nothing.jpg"
            if self.operation == "rename_age": # Rename
                path_new = self.String_ReName(sort_age, key, number, i, "age")
                QtCore.qDebug(str(path_new))
                # path_new = "C:\\Users\\EyeOd\\Desktop\\nothing.jpg"
