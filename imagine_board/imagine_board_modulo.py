...
def Valid_Start(self, start_x, start_y, current_item, others_pack ):
    ...
    if len(others_pack) > 0:
        ...
        for g in range(0, len(grid)):
            ...
            g_l = int(entry[1]) # x
            g_t = int(entry[2]) # Y
            g_r = int(g_l + i_w) # Next Item Width
            g_b = int(g_t + i_h) # Next Item Height

            # Cycle
            valid = True
            for p in range(0, len(others_pack)):
                # Read
                p_l = int(others_pack[p][self.k_bl])
                p_t = int(others_pack[p][self.k_bt])
                p_r = int(others_pack[p][self.k_br])
                p_b = int(others_pack[p][self.k_bb])
    ...
    for g in range(0, len(grid)):
        ...
        if entry not in eliminate:
            for p in range(0, len(others_previous)):
                # Read
                p_l = int(others_previous[p][self.k_bl])
                p_t = int(others_previous[p][self.k_bt])
                p_r = int(others_previous[p][self.k_br])
                p_b = int(others_previous[p][self.k_bb])
