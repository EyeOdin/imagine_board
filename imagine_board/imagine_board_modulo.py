...
class ImagineBoard_Preview(QWidget):
    ...
    SIGNAL_ANIM_ENABLE = QtCore.pyqtSignal(bool)
    ...
    def Set_Sequence_Preview(self, path, playpause):
        ...
        if qmovie.isValid() == True:
            ...
            self.Anim_Play()
            self.update()
    def Set_ZIP_Preview(self, path, file_normal):
        # Reset
        self.Set_Reset()
        # Variables
        self.path = path
    ...
    # Sequence
    def Anim_Enable(self, boolean):
        self.SIGNAL_ANIM_ENABLE.emit(boolean)
    def Anim_Frame(self):
    ...
    def Image_Calculations(self):
        ...
        if (self.is_animation == True and self.is_compressed == False):
            self.image_width = self.sequence[0].width()
            self.image_height = self.sequence[0].height()
        elif (self.is_animation == True and self.is_compressed == True):
            self.image_width = self.sequence[self.frame_value].width()
            self.image_height = self.sequence[self.frame_value].height()
        else:
