...
class ImagineBoard_Function(QWidget):
    SIGNAL_CONTEXT = QtCore.pyqtSignal(str)

    def __init__(self, parent):
        super(ImagineBoard_Function, self).__init__(parent)
    def sizeHint(self):
    ...
    def contextMenuEvent(self, event):
        if event.modifiers() == QtCore.Qt.NoModifier:
            ...
            # Function Key
            cmenu_op = cmenu.addMenu("Operation")
            cmenu_op_key_none = cmenu_op.addAction(">>")
            cmenu_op_key_add = cmenu_op.addAction("Key Add")
            cmenu_op_key_replace = cmenu_op.addAction("Key Replace")
            cmenu_op_key_remove = cmenu_op.addAction("Key Remove")
            cmenu_op_key_clean = cmenu_op.addAction("Key Clean")
            cmenu_op_key_echo = cmenu_op.addAction("Key Echo")
            cmenu_op_rename_order = cmenu_op.addAction("Rename Order")
            cmenu_op_rename_random = cmenu_op.addAction("Rename Random")
            cmenu_op_save_order = cmenu_op.addAction("Save Order")
            # Utilities
            cmenu_util = cmenu.addMenu("Utilities")
            cmenu_util_string = cmenu_util.addAction("String")
            cmenu_util_number = cmenu_util.addAction("Number")
            cmenu_util_path = cmenu_util.addAction("Path")
            # Run
            cmenu_run = cmenu.addAction("Run >>")

            # Map
            action = cmenu.exec_(self.mapToGlobal(event.pos()))

            # Function Key
            if action == cmenu_op_key_none:
                self.SIGNAL_CONTEXT.emit("o.>>")
            if action == cmenu_op_key_add:
                self.SIGNAL_CONTEXT.emit("o.key_add")
            if action == cmenu_op_key_replace:
                self.SIGNAL_CONTEXT.emit("o.key_replace")
            if action == cmenu_op_key_remove:
                self.SIGNAL_CONTEXT.emit("o.key_remove")
            if action == cmenu_op_key_clean:
                self.SIGNAL_CONTEXT.emit("o.key_clean")
            if action == cmenu_op_key_echo:
                self.SIGNAL_CONTEXT.emit("o.key_echo")
            if action == cmenu_op_rename_order:
                self.SIGNAL_CONTEXT.emit("o.rename_order")
            if action == cmenu_op_rename_random:
                self.SIGNAL_CONTEXT.emit("o.rename_random")
            if action == cmenu_op_save_order:
                self.SIGNAL_CONTEXT.emit("o.save_order")
            # Utilities
            if action == cmenu_util_string:
                self.SIGNAL_CONTEXT.emit("s.")
            if action == cmenu_util_number:
                self.SIGNAL_CONTEXT.emit("n.")
            if action == cmenu_util_path:
                self.SIGNAL_CONTEXT.emit("p.")
            if action == cmenu_run:
                self.SIGNAL_CONTEXT.emit("run>>")
