...
class ImagineBoard_Preview(QWidget):
    ...
    SIGNAL_NEUTRAL = QtCore.pyqtSignal(bool)
    # Preview
    ...
    def Set_QPixmap_Preview(self, path):
        ...
        self.update()
    def Set_Sequence_Preview(self, path, playpause):
        self.Preview_Reset(False)
        self.Clip_Off()
        self.path = path
        # Qmovie
    ...
    def Anim_Frame_Save(self, frame):
        ...
        # Save File
        screenshot_qpixmap = self.sequence[frame]
    ...
    def Preview_Reset(self, bool):
        self.SIGNAL_ANIM_PANEL.emit(bool)
        self.zoom = 1
        self.focus_x = 0.5
        self.focus_y = 0.5
        self.frame_value = 0
    ...
    def mousePressEvent(self, event):
        ...
        if (event.modifiers() == QtCore.Qt.ControlModifier and (event.buttons() == QtCore.Qt.LeftButton or event.buttons() == QtCore.Qt.RightButton)):
            self.Preview_Reset(self.is_animation)
            self.Clip_Off()
    ...
    def mouseReleaseEvent(self, event):
        self.SIGNAL_NEUTRAL.emit(False)
        self.origin_x = 0
        self.origin_y = 0
        self.press = False
        self.drop = False
    ...
    def Color_Apply(self, event):
        if self.pick_color == True:
            ...
            blue = color.blue()/255
            dictionary = {"red":red, "green":green, "blue":blue}
    ...
    def paintEvent(self, event):
        ...
        # Display Pixmap
        if (self.qpixmap.isNull() == True and self.is_animation == False and self.drop == False):
            painter.setPen(QtCore.Qt.NoPen)
            ...
            if self.is_animation == True:
                painter.drawPixmap(0,0,self.sequence[self.frame_value])
            else:
    ...
    def Image_Calculations(self):
        ...
        if self.is_animation == True:
            self.image_width = self.sequence[0].width()
            self.image_height = self.sequence[0].height()
        else:
    ...
    def Context_Menu(self, event):
        ...
        if (event.modifiers() == QtCore.Qt.NoModifier and name_path[0] != "."):
            ...
            if action == cmenu_function:
                self.SIGNAL_FUNCTION.emit([name_path])
            if action == cmenu_pin:
                self.SIGNAL_PIN.emit(name_path[1])
            if action == cmenu_document:
    ...
    def paintEvent(self, event):
        ...
        for i in range(0, len(self.ref)):
            ...
            if qpixmap.isNull() == False:
                ...
                painter.drawLine(px,    py,    px+dw, py)
                painter.drawLine(px+dw, py,    px+dw, py+dh)
                painter.drawLine(px+dw, py+dh, px,    py+dh)
                painter.drawLine(px,    py+dh, px,    py)
                painter.drawLine(px,    py,    px+dw, py+dh)
                painter.drawLine(px,    py+dh, px+dw, py)
