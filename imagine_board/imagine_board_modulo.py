...
class ImagineBoard_Reference(QWidget):
    ...
    def Pin_Add(self, pin):
        ...
        qpin = [
            ...
            edit, QPixmap(pin[0]) # Created
            ]
        # Add to list
        self.ref.append(qpin)
        self.update()
    ...
    def Pin_NewPath(self, index):
        ...
        file_path = path.getOpenFileName(self, "Select File", os.path.dirname(self.ref[index][0]) )[0]
        QtCore.qDebug(str(file_path))
        file_path = os.path.normpath( file_path )
        if (file_path != "" and file_path != "."):
            self.Pin_Clean(self.index)
            self.Pin_Drop(file_path)
    def Pin_Zoom(self, bool, path):
        self.pin_zoom = [bool, path]
        self.update()
    ...
    def Pin_Edit(self, operation, index):
        # Load
        qimage = QImage(self.ref[index][0])
        # Edit Operations
        e1 = self.ref[index][-2]["grayscale"]
        e2 = self.ref[index][-2]["invert_h"]
        e3 = self.ref[index][-2]["invert_v"]
        if operation == "grayscale":
            e1 = not e1
            self.ref[index][-2]["grayscale"] = e1
        if operation == "invert_h":
            e2 = not e2
            self.ref[index][-2]["invert_h"] = e2
        if operation == "invert_v":
            e3 = not e3
            self.ref[index][-2]["invert_v"] = e3
        # Grayscale
        if e1 == True:
            value = 24
        else:
    ...
    def Board_Save(self):
        ...
        for i in range(0, len(self.ref)):
            reference.append(self.ref[i][0:-2]) # Cleans QPixmap excess meant for render only
        self.SIGNAL_UPDATE.emit([self.kra_bind, reference])
    ...
    def Selection_Square(self, event):
        ...
        for i in range(0, len(self.ref)):
            if (
            self.ref[i][1] >= square_top_left_x and
            self.ref[i][2] >= square_top_left_y and
            (self.ref[i][1] + self.ref[i][3]) <= square_bot_right_x and
            (self.ref[i][2] + self.ref[i][4]) <= square_bot_right_y):
                self.selection.append(self.ref[i][0])
        self.update()
    ...
    def Pack_Straight(self, orientation):
        ...
        sort = []
        if orientation == "line":
            sort = self.Selection_Sort(self.selection, 4) # Sort by height. height=4
        if orientation == "column":
            sort = self.Selection_Sort(self.selection, 3) # Sort by width. width=3
        ...
        for s in range(1, len(sort)):
            i = s - 1
            if orientation == "line":
                sort[s][1] = sort[i][1] + sort[i][3]
                sort[s][2] = mpy
            if orientation == "column":
                sort[s][1] = mpx
                sort[s][2] = sort[i][2] + sort[i][4]
        # Copy values back to References List
        for r in range(0, len(self.ref)):
            for a in range(0, len(sort)):
                if self.ref[r][0] == sort[a][0]:
                    self.ref[r] = sort[a]
        # Update the Display
        self.update()
    def Pack_Optimized(self):
        # Create Sort List
        sort = []
        sort = self.Selection_Sort(self.selection, 8) # Sort by height. height=4
        mpx, mpy, mdx, mdy = self.Pixmap_Area(sort)
        QtCore.qDebug(str(mpx))
        QtCore.qDebug(str(mpy))

        # Totals
        delta_x = 0
        delta_y = 0
        for i in range(0, len(self.ref)):
            if self.ref[i][0] in self.selection:
                delta_x += self.ref[i][3]
        for i in range(0, len(self.ref)):
            if self.ref[i][0] in self.selection:
                delta_y += self.ref[i][4]
        total_x = mpx + delta_x
        total_y = mpy + delta_y
        QtCore.qDebug("total_x = "+str(total_x))
        QtCore.qDebug("total_y = "+str(total_y))

        # Initialize Variables
        sort[0][1] = mpx
        sort[0][2] = mpy
        grid_x = [mpx, mpx + sort[0][3]]
        grid_y = [mpy, mpy + sort[0][4]]

        points = [
                [
                sort[0][0], # path of the image as an identifier
                sort[0][1], sort[0][2], # Point X Y
                sort[0][3], sort[0][4], # Width and Height
                sort[0][1]+sort[0][3], sort[0][2]+sort[0][4], # Point XX YY
        ...
        # # Sorting
        # for s in range(1, len(sort)):
        #     QtCore.qDebug("-------- New Image")
        #     QtCore.qDebug("s="+str(a)+" "+str(os.path.basename(sort[s][0]) ) )
        #     QtCore.qDebug("grid_y = "+str(grid_y))
        #     QtCore.qDebug("grid_x = "+str(grid_x))
        #
        #     for y in range(0, len(grid_y)):
        #         QtCore.qDebug("y= "+str(y)+"--")
        #         for x in range(0, len(grid_x)):
        #             QtCore.qDebug("x= "+str(x)+"--")
        #
        #             # Check Values
        #             point_sx = grid_x[x]
        #             point_sy = grid_y[y]
        #             delta_sx = point_sx + sort[s][3]
        #             delta_sy = point_sy + sort[s][4]
        ...
        #     sort[s][2] = point_fy
        #     # Adjust Grid
        #     if point_fx not in grid_x:
        #         grid_x.append(point_fx)
        #     if point_fy not in grid_y:
        #         grid_y.append(point_fy)
        #     if delta_fx not in grid_x:
        #         grid_x.append(delta_fx)
        #     if delta_fy not in grid_y:
        #         grid_y.append(delta_fy)
        #     grid_x.sort()
        #     grid_y.sort()
        #
        #     QtCore.qDebug("-------- End")
            ...
            for i in range(0, len(initial)):
                if topic[t] == initial[i][index]:
                    ...
                    for s in range(0, len(sorted)):
                        if intial_i[0] == sorted[s][0]:
                            duplicates = True
                    if duplicates == False:
                        sorted.append(intial_i)
        return sorted
    ...
    def Index_Closest(self, event):
        ...
        self.deltas = []
        for i in range(0, len(self.ref)):
            delta_x = self.origin_x - self.ref[i][1]
            delta_y = self.origin_y - self.ref[i][2]
    ...
    def Index_Node(self, event):
        # Node Choice
        nodes = self.Index_Nodes(self.index)
        check = []
    ...
    def Pixmap_Move(self, event, index, selection):
        ...
        if index != None:
            if len(self.limit_x) > 0:
                for i in range(0, len(self.limit_x)):
                    ...
                    if (nl >= ll and nl <= lr):
                        self.ref[index][1] = self.limit_x[i]
                        sx = self.ref[index][1] - mx
                        break
                    elif (nr >= ll and nr <= lr):
                        self.ref[index][1] = self.limit_x[i] - self.ref[index][3]
                        sx = self.ref[index][1] - mx
                        break
            if len(self.limit_y) > 0:
                for i in range(0, len(self.limit_y)):
                    ...
                    if (nt >= lt and nt <= lb):
                        self.ref[index][2] = self.limit_y[i]
                        sy = self.ref[index][2] - my
                        break
                    elif (nb >= lt and nb <= lb):
                        self.ref[index][2] = self.limit_y[i] - self.ref[index][4]
                        sy = self.ref[index][2] - my
                        break
        ...
        if sel_len > 1:
            for i in range(0, len(self.ref)):
                if (self.ref[i][0] in selection and self.ref[i][0] != self.ref[index][0]):
                    self.ref[i][1] = event_x - self.deltas[i][0] + sx
                    self.ref[i][2] = event_y - self.deltas[i][1] + sy
        # Update
        self.update()
    def Pixmap_Scale(self, event, index, node):
        ...
        if node == 1:
            ...
            self.ref[index][1] = nx - tx
            self.ref[index][2] = ny - ty
            self.ref[index][3] = tx
            self.ref[index][4] = ty
            # Snapping
            if (len(self.limit_x) > 0 and len(self.limit_y) > 0):
        ...
        if node == 9:
            ...
            self.ref[index][3] = math.cos(math.radians(ang)) * hip
            self.ref[index][4] = math.sin(math.radians(ang)) * hip
            # Snapping
            if (len(self.limit_x) > 0 and len(self.limit_y) > 0):
        ...
        self.ref[index][6] = (2 * self.ref[index][3]) + (2 * self.ref[index][4])
        self.ref[index][7] = self.ref[index][3] * self.ref[index][4]
        self.update()
    ...
    def Camera_Scale(self, event):
        ...
        value = 0.01
        for i in range(0, len(self.ref)):
            self.ref[i][1] = self.camera_scale[i][0] + (-self.origin_x + self.camera_scale[i][0]) * (-dist * value)
            self.ref[i][2] = self.camera_scale[i][1] + (-self.origin_y + self.camera_scale[i][1]) * (-dist * value)
            self.ref[i][3] = (self.camera_scale[i][4] + (-self.origin_x + self.camera_scale[i][4]) * (-dist * value)) - self.ref[i][1]
            self.ref[i][4] = (self.camera_scale[i][5] + (-self.origin_y + self.camera_scale[i][5]) * (-dist * value)) - self.ref[i][2]
            self.ref[i][6] = (2 * self.ref[i][3]) + (2 * self.ref[i][4])
            self.ref[i][7] = self.ref[i][3] * self.ref[i][4]
        self.update()
    ...
    def mousePressEvent(self, event):
        ...
        if (event.buttons() == QtCore.Qt.LeftButton and event.modifiers() == (QtCore.Qt.ShiftModifier | QtCore.Qt.ControlModifier)): # Pin Zoom
            ...
            # Open Pin Zoom
                ...
                self.Index_Closest(event)
                if self.index is not None:
                    self.Pin_Zoom(True, self.ref[self.index][0])
    ...
    def mouseMoveEvent(self, event):
        ...
        if (event.buttons() == QtCore.Qt.LeftButton and event.modifiers() == QtCore.Qt.AltModifier): # Drag and Drop
            if self.index is not None:
                self.SIGNAL_DRAG.emit(self.ref[self.index][0])
    ...
    def Wheel_Scale(self, event, value):
        ...
        ey = event.y()
        for i in range(0, len(self.ref)):
            self.ref[i][1] = self.camera_scale[i][0]  + (-ex + self.camera_scale[i][0]) * (value)
            self.ref[i][2] = self.camera_scale[i][1]  + (-ey + self.camera_scale[i][1]) * (value)
            self.ref[i][3] = (self.camera_scale[i][4] + (-ex + self.camera_scale[i][4]) * (value)) - self.ref[i][1]
            self.ref[i][4] = (self.camera_scale[i][5] + (-ey + self.camera_scale[i][5]) * (value)) - self.ref[i][2]
            self.ref[i][6] = (2 * self.ref[i][3]) + (2 * self.ref[i][4])
            self.ref[i][7] = self.ref[i][3] * self.ref[i][4]
            ...
            if self.pin_zoom[0] == True:
                ...
                if self.index == None:
                    ...
                    if sel_len > 1:
                        ...
                        cmenu_edit_greyscale.setCheckable(True)
                        cmenu_edit_greyscale.setChecked(self.ref[self.index][-2]["grayscale"])
                        cmenu_edit_invert_h = cmenu.addAction("Edit Flip Horizontal")
                        cmenu_edit_invert_h.setCheckable(True)
                        cmenu_edit_invert_h.setChecked(self.ref[self.index][-2]["invert_h"])
                        cmenu_edit_invert_v = cmenu.addAction("Edit Flip Vertical")
                        cmenu_edit_invert_v.setCheckable(True)
                        cmenu_edit_invert_v.setChecked(self.ref[self.index][-2]["invert_v"])
                        cmenu.addSeparator()
                        ...
                        if action == cmenu_zoom_open:
                            self.Pin_Zoom(True, self.ref[self.index][0])
                        if action == cmenu_edit_greyscale:
                            self.Pin_Edit("grayscale", self.index)
                        if action == cmenu_edit_invert_h:
                            self.Pin_Edit("invert_h", self.index)
                        if action == cmenu_edit_invert_v:
                            self.Pin_Edit("invert_v", self.index)
    ...
    def paintEvent(self, event):
        ...
        for i in range(0, len(self.ref)):
            ...
            if image.isNull() == False:
                painter.setPen(QtCore.Qt.NoPen)
                painter.drawPixmap(px, py, image.scaled(sx, sy, Qt.IgnoreAspectRatio, Qt.SmoothTransformation))
            else:
        ...
        for i in range(0, len(self.ref)):
            if self.ref[i][0] in self.selection:
                painter.drawRect(self.ref[i][1], self.ref[i][2], self.ref[i][3], self.ref[i][4])
        ...
        if (self.active == True and self.index is not None and len(self.ref) > 0):
            # Squares
            minimal_triangle = 20
            if (self.ref[self.index][3] > minimal_triangle and self.ref[self.index][4] > minimal_triangle):
                # Variables
                painter.setBrush(QBrush(self.color1, Qt.SolidPattern))
            ...
            # Squares
            minimal_square = 50
            if (self.ref[self.index][3] > minimal_square and self.ref[self.index][4] > minimal_square):
                # Variables
                n2_x =  px + (sx * 0.5)
                n2_y =  py
                n4_x =  px
