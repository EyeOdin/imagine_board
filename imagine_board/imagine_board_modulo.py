...
class ImagineBoard_Reference(QWidget):
    ...
    def Pack_Optimized(self):
        # Create Sort List
        sort = []
        sort = self.Selection_Sort(self.selection, 6) # Sort by height. height=4
        mpx, mpy, mdx, mdy = self.Pixmap_Area(sort)
        ...
        # Sorting
        for a in range(1, len(sort)):
            QtCore.qDebug("-------- New Image")
            QtCore.qDebug("a="+str(a)+" "+str(os.path.basename(sort[a][0]) ) )
            QtCore.qDebug("grid_y = "+str(grid_y))
            QtCore.qDebug("grid_x = "+str(grid_x))

            for v in range(0, len(grid_y)):
                QtCore.qDebug("v= "+str(v)+"--")
                for u in range(0, len(grid_x)):
                    QtCore.qDebug("u= "+str(u)+"--")

                    # Check Values
                    point_au = grid_x[u]
                    point_av = grid_y[v]
                    delta_au = point_au + sort[a][3]
                    delta_av = point_av + sort[a][4]

                    # Check if placing it would overlap previously placed pixmaps
                    before = a
                    QtCore.qDebug("  before "+str(before))
                    for b in range(0, before):
                        QtCore.qDebug("  b = "+str(b))
                        point_bu = sort[b][1]
                        point_bv = sort[b][2]
                        delta_bu = point_bu + sort[b][3]
                        delta_bv = point_bv + sort[b][4]

                        inside_area = (
                            ((point_au >= point_bu and point_au < delta_bu) and (point_av >= point_bv and point_av < delta_bv)) or
                            ((delta_au >  point_bu and delta_au < delta_bu) and (delta_av >  point_bv and delta_av < delta_bv)) )
                        QtCore.qDebug("    inside_area = "+str(inside_area))
                        if inside_area == True:
                            QtCore.qDebug("    Overlaps")
                        if inside_area == False:
                            QtCore.qDebug("    Free Space")
                            # Check Compactness
            ...
            sort[a][2] = point_fv
            # Adjust Grid
            if point_fu not in grid_x:
                grid_x.append(point_fu)
            if point_fv not in grid_y:
                grid_y.append(point_fv)
            if delta_fu not in grid_x:
                grid_x.append(delta_fu)
            if delta_fv not in grid_y:
                grid_y.append(delta_fv)
            grid_x.sort()
            grid_y.sort()
        ...
        if event.modifiers() == QtCore.Qt.NoModifier:
            ...
            if self.index == None:
                ...
                if sel_len > 1:
                    ...
                    # Actions
                    cmenu_pin_zoom = cmenu.addAction("Pin Zoom")
