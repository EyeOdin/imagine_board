...
class ImagineBoard_Reference(QWidget):
    ...
    def Pack_Optimal_Start(self, method, mode):
        ...
        self.thread_packer.SIGNAL_PB_VALUE.connect(self.Pack_Progress_Value)
        self.thread_packer.SIGNAL_PB_MAX.connect(self.Pack_Progress_Maximum)
        ...
        self.thread_packer.setPriority( QThread.TimeCriticalPriority )
    def Pack_Optimal_Ref(self, pin):
        ...
        self.modified = True
        self.update()
    def Pack_Progress_Value(self, value):
        self.layout.progress_bar.setValue( value )
    def Pack_Progress_Maximum(self, maximum):
    ...
    def paintEvent(self, event):
        ...
        painter.setPen(QtCore.Qt.NoPen)
        if self.packing == True:
            # painter.setBrush(QBrush(self.color_blue, Qt.Dense6Pattern))
...
class Thread_Packer(QThread):
    ...
    def run(self):
        ...
        # Print
        try:QtCore.qDebug("IB " + str(time.toString('hh:mm:ss.zzz')) + " | Count " + str(count) + " | Packer " + str(self.mode) + " | Efficiency " + str(report) + "%")
        except:pass
    ...
    def Pack_Optimal(self, pin_ref, mode):
        ...
        for s in range(0, amount):
            ...
            if s == 0:
                ...
                eliminate = []
                blacklist = []
                points_x, points_y, points_v, eliminate, blacklist = self.Valid_Points(organized, eliminate, blacklist)
            else:
                # Variables
                control = []
                descrim_1 = []
                ...
                for p in range(0, len(points_v)):
                    ...
                    for v in range(0, s):
                        ...
                        # Checks for overlaps
                        overlap = ( ( ipxw <= ivbl or ipx >= ivbr ) or ( ipyh <= ivbt or ipy >= ivbb ) )
    ...
    def Valid_Points(self, lista, eliminate, blacklist):
        # Construct Intersections
        set_x = set()
        set_y = set()
        for i in range(0, len(lista)):
            set_x.add( lista[i]["bl"] )
            set_x.add( lista[i]["br"] )
            set_y.add( lista[i]["bt"] )
            set_y.add( lista[i]["bb"] )
        points_x = list(set_x)
        points_y = list(set_y)
        ...
        for p in range(0, len(grid)):
            ...
            if entry not in eliminate:
                ...
                for l in range(0, len(lista)):
                    ...
                    bb = int(lista[l]["bb"])
                    # Pixel Check
                    if ( px == bl and py == bt ): # point is insde the top+left point of a image
                        eliminate.append( entry )
                        blacklist.append( entry )
                    if ( ( px >= bl and px <= (br-num) ) and ( py >= bt and py <= (bb-num) ) ): # point is inside the draw pixmap
                        eliminate.append( entry )
        ...
        for y in range(0, len(points_y)):
            ...
            if entry not in eliminate:
                extra.append( entry )
        # Final
        points_valid.extend(extra)
        # Sort
        points_valid.sort(reverse=False, key=self.Sort_Dist_Value)

        # Returns
        return points_x, points_y, points_valid, eliminate, blacklist
