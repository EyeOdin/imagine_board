...
def event_drop( self, event, image_path ):
    ...
    # Return
    return mime_data
def Neutral_Press( self, event, press, path ):
    # Color Picker
    px, py, red, green, blue = PickColor_Event( self, event )
    # Text
    text = ""
    if press == True:
    ...
    # Signals
    if self.pick_color == True:
        self.SIGNAL_COLOR.emit( { "press":press, "red":red, "green":green, "blue":blue } )
    self.SIGNAL_PRESS.emit( { "press":press, "picker":self.pick_color, "px":px, "py":py, "size":colorpicker_size } )
    self.SIGNAL_TEXT.emit( text )
    # Update
    self.update()
def PickColor_Event( self, event ):
    # Event
    ex = Limit_Range( event.x(), 0, self.widget_width-1 )
    ey = Limit_Range( event.y(), 0, self.widget_height-1 )
    # Picker Location
    pos = QPoint( ex, ey )
    ...
    blue = pixel.blueF()
    # Return
    return px, py, red, green, blue

def Display_ColorPicker( self, event, painter, ex, ey, color_active, color_previous ):
    # Values
    if ex <= 0:
        ex = 0
    if ey <= 0:
        ey = 0
    if ex >= self.widget_width:
        ex = self.widget_width
    if ey >= self.widget_height:
        ey = self.widget_height
    ...
    # Color Previous
    painter.setPen( QtCore.Qt.NoPen )
    painter.setBrush( QBrush( color_active ) )
    painter.drawEllipse( ex, ey, colorpicker_size, colorpicker_size )
...
class ImagineBoard_Preview( QWidget ):
    ...
    SIGNAL_PRESS = QtCore.pyqtSignal( dict )
    ...
    def Set_Color_Active( self, color_press, qcolor  ):
        self.color_press = color_press
        self.color_active = qcolor
        self.update()
    def Set_Color_Previous( self, color_press, qcolor  ):
        self.color_press = color_press
    ...
    def mousePressEvent( self, event ):
        ...
        if ( event.modifiers() == QtCore.Qt.NoModifier and event.buttons() == QtCore.Qt.LeftButton ):
            Neutral_Press( self, event, True, os.path.basename( self.path ) )
    ...
    def mouseMoveEvent( self, event ):
        # Read
        self.pixel_x = event.x()
        self.pixel_y = event.y()
    ...
    def mouseReleaseEvent( self, event ):
        # Event
        Neutral_Press( self, event, False, None )
        # Variables
        self.origin_x = 0
        self.origin_y = 0
        self.press = False
    ...
    def Context_Menu( self, event ):
        ...
        if ( self.pigmento == True and is_compressed == False ):
    ...
    def paintEvent( self, event ):
        ...
        if self.color_press == True:
            Display_ColorPicker( self, event, painter, self.pixel_x, self.pixel_y, self.color_active, self.color_previous )
...
class ImagineBoard_Grid( QWidget ):
    ...
    def mousePressEvent( self, event ):
        ...
        if ( event.modifiers() == QtCore.Qt.NoModifier and event.buttons() == QtCore.Qt.LeftButton ):
            ...
            Neutral_Press( self, event, True, basename )
        ...
        if self.press == None:
            ...
            if ( event.buttons() == QtCore.Qt.LeftButton and event.modifiers() == QtCore.Qt.NoModifier ):
                ...
                if self.pick_color == False:
                    if self.pin_index != None:
                        basename = os.path.basename( self.pin_ref[self.pin_index]["path"] )
...
class ColorPicker_Display( QWidget ):

    def __init__( self, parent ):
        super( ColorPicker_Display, self ).__init__( parent )
        # Variables
        self.color = QColor( 0, 0, 0 )
        self.previous = QColor( 0, 0, 0 )
    def sizeHint( self ):
        return QtCore.QSize( 5000, 5000 )
    ...
    def Set_Color( self, red, green, blue ):
        ...
        self.update()
    def Set_Previous( self, red, green, blue ):
        self.previous = QColor( red*255, green*255, blue*255 )
        self.update()
    ...
    def paintEvent( self, event ):
        ...
        # Mask
        mask = QPainterPath()
        mask.addEllipse( 0, 0, colorpicker_size, colorpicker_size )
        mask.addEllipse( cps_c, cps_c, cps_d, cps_d )
        painter.setClipPath( mask )
        # Neutral
        painter.setPen( QtCore.Qt.NoPen )
        painter.setBrush( QBrush( QColor( 128, 128, 128 ) ) )
        painter.drawEllipse( 0, 0, colorpicker_size, colorpicker_size )
        ...
        painter.drawEllipse( cps_a, cps_a, cps_b, cps_b )

        # Mask
        square = QPainterPath()
        square.addRect( 0, 0, colorpicker_size, colorpicker_size*0.5 )
        circle = QPainterPath()
        circle.addEllipse( 0, 0, colorpicker_size, colorpicker_size )
        circle.addEllipse( cps_c, cps_c, cps_d, cps_d )
        half_ring = circle.subtracted( square )
        painter.setClipPath( half_ring )
        # Color
        painter.setPen( QtCore.Qt.NoPen )
        painter.setBrush( QBrush( self.previous ) )
        painter.drawEllipse( cps_a, cps_a, cps_b, cps_b )
